...
public class ImagePickerPlusActivity extends BaseActivity implements
		OnClickListener {
	...
	// public static final String EXTRA_PICK_RETURN_DATA_ORIENTATIONS =
	// "photoOrientations";
	...
	private ArrayList<String> choicePhotoPaths = new ArrayList<String>();
	private ArrayList<ItemImageInfo> datas = new ArrayList<ItemImageInfo>();
			...
			if (adapter instanceof MyCursorAdapter) {
				try {
					((MyCursorAdapter) adapter)
							.changeCursor(getAlbumnAdapterDatas());
				} catch (Exception e) {
	...
	private void setLayoutHeight(ImageView imageView) {
		...
		if (vl == null) {
			vl = new ViewGroup.LayoutParams(imgViewWidthAndHeight,
					imgViewWidthAndHeight);
		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			itemAlbumDatas = new LongSparseArray<AlbumInfo>();
		} catch (Exception e) {
			LogUtil.w(
					TAG,
					"getAdapterDatas() SQL inject maybe fail ! getDataFrom getAllAdapterDatas()",
					e);
	...
	class MyBaseAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (view == null) {
				view = layoutInfalter.inflate(R.layout.list_grid_item, parent,
						false);
			}
			...
			ImageView imageView2 = (ImageView) view
					.findViewById(R.id.image_item_2);
			...
			TextView blackBg2 = (TextView) view
					.findViewById(R.id.tv_checked_bg2);
			TextView blackBg3 = (TextView) view
					.findViewById(R.id.tv_checked_bg3);
			...
			if (isAlbumMode) {
				...
				if (position * 3 < dataSize) {
					...
					t.addTask(imgInfo.filePath, imageView, imgInfo.imageId,
							imgInfo.orientation);
					bottomCt.setVisibility(View.VISIBLE);
				...
				if (position * 3 + 1 < dataSize) {
					...
					imgInfo = albumInfo.getConver();
					t.addTask(imgInfo.filePath, imageView2, imgInfo.imageId,
							imgInfo.orientation);
					bottomCt2.setVisibility(View.VISIBLE);
				...
				if (position * 3 + 2 < dataSize) {
					...
					imgInfo = albumInfo.getConver();
					t.addTask(imgInfo.filePath, imageView3, imgInfo.imageId,
							imgInfo.orientation);
					bottomCt3.setVisibility(View.VISIBLE);
				...
				if (position * 3 < dataSize) {
					...
					if (imgInfo.isChecked) {
						cb.setChecked(true);
						blackBg.setVisibility(View.VISIBLE);
						// blackBg.setText(getSizeStr(imgInfo.size,
						// imgInfo.filePath));
					} else {
						cb.setChecked(false);
						blackBg.setVisibility(View.GONE);
				...
				if (position * 3 + 1 < dataSize) {
					...
					if (imgInfo.isChecked) {
						cb2.setChecked(true);
						blackBg2.setVisibility(View.VISIBLE);
						// blackBg2.setText(getSizeStr(imgInfo.size,
						// imgInfo.filePath));
					} else {
				...
				if (position * 3 + 2 < dataSize) {
					...
					if (imgInfo.isChecked) {
						cb3.setChecked(true);
						blackBg3.setVisibility(View.VISIBLE);
						blackBg3.setText(getSizeStr(imgInfo.size,
								imgInfo.filePath));
					} else {
	...
	private View.OnClickListener onItemClick = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (o instanceof String) {
				...
				Intent intent = new Intent(ImagePickerPlusActivity.this,
						LargePreviewActivity.class);
				...
				if (adapter instanceof MyCursorAdapter) {
					((MyCursorAdapter) adapter)
							.changeCursor(getPhotoInfoCursorByAlbumId(clickAlbumId));
				} else {
					adapter.notifyDataSetChanged();
				}
				if (!isAlbumMode) {
	...
	class MyCursorAdapter extends CursorAdapter {
		...
		public View newView(Context context, Cursor cursor, ViewGroup parent) {
			return layoutInflater.inflate(R.layout.list_grid_item, parent,
					false);
		}
			...
			if (isAlbumMode) {
				...
				if (currentPosition * 3 < size) {
					...
					if (albumInfo == null) {
						...
						conver.orientation = new String(String.valueOf(cursor
								.getInt(6)));
				...
				if (currentPosition * 3 < size) {
					...
					if (null == imgInfo) {
						...
						imgInfo.orientation = new String(String.valueOf(cursor
								.getInt(3)));
					...
					// imgInfo = albumInfo.getImageInfos().get(currentPosition *
					// 3);
					t.addTask(imgInfo.filePath, imageView, imgInfo.imageId,
							imgInfo.orientation);
					allCt.setVisibility(View.VISIBLE);
	...
	public static String getSizeStr(long size) {
		if (size > 0) {
			...
			if (temp > 1024.0d) { // > 1MB
				...
				imgSize = sizeStr.substring(0, sizeStr.indexOf(".") + 2)
						+ " MB";
			} else {
		...
		public void onCheckedChanged(CompoundButton buttonView,
			...
			if (choicePhotoPaths.size() >= CAN_CHECK_COUNT && isChecked) {
				...
				Toast.makeText(getApplicationContext(),
						"最多选择" + CAN_CHECK_COUNT + "张，编辑时可以添加。",
			...
			if (null != position) {
				AlbumInfo albumInfo = itemAlbumDatas.get(clickAlbumId);
				ItemImageInfo imgItemInfo = albumInfo
						.getImageInfoByIndex(position);
				TextView tvBg = (TextView) buttonView
						.getTag(R.string.view_tag_key);
				if (isChecked) {
					tvBg.setVisibility(View.VISIBLE);
					// tvBg.setText(getSizeStr(imgItemInfo.size,
					// imgItemInfo.filePath));
				...
				titleTv.setText("选择图片 (" + choicePhotoPaths.size() + "/"
						+ CAN_CHECK_COUNT + ")");
				...
				for (int i = 0; i < choicePhotoSize; i++) {
					allFileSize = allFileSize
							+ new File(choicePhotoPaths.get(i)).length();
				}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case CODE_LARGE_PREVIEW:
			if (resultCode == RESULT_OK) {
				...
				Integer cbResId = (Integer) clickItemView
						.getTag(R.string.view_tag_key);
				if (null != cbResId) {
					CheckBox checkBox = (CheckBox) clickItemView
							.findViewById(cbResId);
	...
	class LoadPhonePhotoThread extends Thread {
		...
		private LongSparseArray<String> thumbnailsMap = new LongSparseArray<String>();
		...
		public void addTask(String filePath, ImageView imgView, Long imgId,
				String orientation) {
			if (imgView != null) {
				synchronized (imgView) {
					if (null != filePath && null != imgId
							&& null != orientation) {
		...
		public void run() {
			...
			Cursor cursor = getContentResolver()
					.query(Thumbnails.EXTERNAL_CONTENT_URI,
							projection,
							Thumbnails.KIND + "=?",
							new String[] { String.valueOf(Thumbnails.MINI_KIND) },
							null);
			...
			while (flag) {
				if (!imgViews.isEmpty()) {
					...
					synchronized (imgView) {
						...
						tagFilePath = (String) imgView
								.getTag(R.string.view_tag_key);
						orientation = (String) imgView
								.getTag(R.string.view_tag_key2);
					}
					Bitmap b = null;
					...
					// gen system mini by system and save to system
					// diskcache、db.
					if (b == null) {
						// 性能很差
		...
		private Bitmap getSystemMiniFromSystemDiskCache(long imgId) {
			...
			if (!TextUtils.isEmpty(thumbnailFilePath)) {
				...
				if (miniFile.exists() && miniFile.length() > 0) {
					...
					if (options.outWidth > imgViewWidthAndHeight
							|| options.outHeight > imgViewWidthAndHeight) {
						final float maxBitmapBorder = options.outWidth > options.outHeight ? options.outWidth
								: options.outHeight;
						options.inSampleSize = Math.round(maxBitmapBorder
								/ ((float) imgViewWidthAndHeight));
					}
		...
		private Bitmap getMyMiniFromMyDiskCache(String oriFilePath) {
			if (!TextUtils.isEmpty(diskCachePath)
					&& !TextUtils.isEmpty(oriFilePath)) {
				Bitmap b = BitmapUtil.getBitmap(new File(diskCachePath,
						new File(oriFilePath).getName().split("\\.")[0])
						.getAbsolutePath());
				if (b == null) {
		...
		private Bitmap getSystemMiniFromSystem(long imgId) {
			...
			options.inJustDecodeBounds = false;
			Bitmap bm = Thumbnails.getThumbnail(getContentResolver(), imgId,
					Thumbnails.MINI_KIND, options);
			if (bm != null && TextUtils.isEmpty(thumbnailsMap.get(imgId))) {
				LogUtil.i(TAG, "4.gen mini and save path.");
				Cursor c = Thumbnails.queryMiniThumbnail(getContentResolver(),
						imgId, Thumbnails.MINI_KIND, new String[] {
								Thumbnails._ID, Thumbnails.DATA });
				if (null != c) {
					if (c.moveToFirst()) {
		...
		private Bitmap getMyMiniFromSystemOri(String filePath) {
			if (!TextUtils.isEmpty(filePath)) {
				...
				if (options.outWidth > imgViewWidthAndHeight
						|| options.outHeight > imgViewWidthAndHeight) {
					float maxBitmapBorder = options.outWidth > options.outHeight ? options.outWidth
							: options.outHeight;
					options.inSampleSize = Math.round(maxBitmapBorder
							/ ((float) imgViewWidthAndHeight));
				}
				...
				if (null != bm) {
					...
					try {
						...
						fos = new FileOutputStream(new File(diskCachePath,
								new File(filePath).getName().split("\\.")[0]));
						bm.compress(Bitmap.CompressFormat.JPEG, 100, fos);
					} catch (Exception e) {
						LogUtil.e(TAG,
								"mini from ori saving to diskcache fail !", e);
					} finally {
						if (null != fos) {
							try {
								fos.close();
	...
	private Cursor getPhotoInfoCursorByAlbumId(long albumId) {
		String[] projection = { Media._ID, Media.SIZE, Media.DATA,
				Media.ORIENTATION };
		Cursor cursor = getContentResolver().query(
				...
				Media.DATE_MODIFIED + " DESC, " + Media.DATE_ADDED + " DESC, "
						+ Media._ID + " DESC");
		return cursor;
	}
	...
	private Cursor getAlbumnAdapterDatas() throws Exception {
		String[] projection = { Media._ID, Media.SIZE, Media.DATA,
				Media.BUCKET_ID, Media.BUCKET_DISPLAY_NAME, Media.DISPLAY_NAME,
				Media.ORIENTATION, "COUNT(0) AS count" };
		Cursor cursor = getContentResolver().query(
				Media.EXTERNAL_CONTENT_URI,
				projection,
	...
	protected void initViews() {
		...
		titleTv.setText("选择图片(0/" + CAN_CHECK_COUNT + ")");
		finishBtn.setEnabled(false);
	}
	...
	public void onClick(View v) {
		...
		case R.id.header_right_btn:
			setResult(RESULT_OK,
					new Intent().putExtra(EXTRA_PICK_RETURN_DATA, datas));
			finish();
			break;
		}
	}
